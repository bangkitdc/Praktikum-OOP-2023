 ...
 *       - Apabila tingkat prioritas dari operator sekarang lebih kecil dari operator 
...
int Calculator::Evaluate(const string& tokens)
{
  ...
  for (int i = 0; i < tokens.length(); i++) {
    ...
    try {
      if (!IsOperator(c) && !IsOperand(c) && (c != '(' && c != ')')) {
